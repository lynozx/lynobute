...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `opa, @${num.split('@')[0]}\nSeja bem vindo *${mdata.subject}*sega as regras, se n vai mama os adm e os membro tudo, ainda vai levar ban 😉, fique a vontade bb`
				...
				teks = `flw seu bosta @${num.split('@')[0]} -1 `
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'To fazendo o bglh, perae',
				success: 'saiu quentinho️',
				error: {
			...
			const ownerNumber = [`558897187831@s.whatsapp.net`] // replace this with your number
			...
			switch(command) {
             ...
             case 'level':
                ...
                resul = `╠━━━━━━━━❮INFO DO USER❯━━━━━━━━║╠➥ *nome* : ${pushname}║╠➥ *ua mi 😁* : wa.me/${sender.split("@")[0]}║╠➥ *XP* : ${getLevelingXp(sender)}/${reqXp}║╠➥ *level* : ${getLevelingLevel(sender)}║╠➥ *user register* : ${_registered.length}║`
               client.sendMessage(from, resul, text, { quoted: mek})
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `⚠ LYNO AVISOS KKK ⚠️️\n\n${body.slice(4)}`})
						}
						reply('tm feita')
					} else {
						for (let _ of anu) {
					...
					if (mentioned.length > 1) {
						...
						mentions(`adm novo @${mentioned[0].split('@')[0]} 😎`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Parabéns @${mentioned[0].split('@')[0]} conseguiu perder adm 👏👏!`, mentioned, true)
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Tchau seu bosta @${mentioned[0].split('@')[0]} foi de ralo 🥺`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
                ...
                case 'leave':
                    ...
                    break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ responder stickernya hum ❌')
					reply(mess.wait)
